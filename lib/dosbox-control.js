...
class DosboxControl {
  constructor(options = {}) {
    ...
    if (!Number.isInteger(this.timeout) || this.timeout < 1) {
      throw new Error('DOSBox-X control timeout must be a positive integer');
    }
    if (!Number.isInteger(this.maxResponseBytes) || this.maxResponseBytes < 1) {
      throw new Error('DOSBox-X maximum response size must be a positive integer');
    }
  }
  ...
  send(command, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.on('end', () => finish(resolve, response.trimEnd()));
      socket.on('timeout', () => {
        socket.destroy();
